        ...
        <div>
            ...
            >
                ...
                >
                    ...
                    <div class="evaluation-header">
                        <h5>
                            <b>{{ event_title["organization"] }}</b>
                        </h5>
                        <h2>
                            {{
                                this.evaluation_with_questions_options
                                    .evaluation_title
                            }}
                        </h2>
                        <h5>
                            Name of Activity: <b>{{ event_title["name"] }}</b>
                        </h5>
                        <h5>
                            Date and Time:
                            <b>{{ event_title["start_attendance"] }}</b>
                        </h5>
                        <h5>
                            Venue: <b>{{ event_title["location"] }}</b>
                        </h5>
                        <hr />
...
export default {
    ...
    computed: {
        totalPages() {
            return Math.ceil(
                this.event_evaluation_form.length / this.itemsPerPage
            );
        },
        currentPageEvaluationForms() {
            const startIndex = (this.currentPage - 1) * this.itemsPerPage;
            const endIndex = startIndex + this.itemsPerPage;
            const currentPageForms = this.event_evaluation_form.slice(
                startIndex,
                endIndex
            );
    ...
    methods: {
        ...
        fetchEvaluation_form() {
            axios
                .get(`/event_evaluation_form/${this.event_id}/${this.org_id}`)
                .then((response) => {
        ...
        submit() {
            ...
            axios
                .post(`/submit_evaluation/${this.user_id}/${this.event_id}`, {
        ...
        showEventTitle() {
            axios
                .get(`/evaluation_form_title/${this.event_id}`)
                .then((response) => {
                    // console.log(response.data)
                    ...
                    data.forEach((item) => {
                        ...
                        this.event_title["organization"] =
                            item["organization"]["name"];
                        this.event_title["start_attendance"] =
                            item["start_attendance"];
        ...
        showQuestions(id) {
            axios
                .get(`/get_evaluation_question/${id}`)
                .then((response) => {
                    console.log(response.data);
                    this.evaluation_with_questions_options = response.data;
                    this.event_title.description =
                        response.data.evaluation_description;
